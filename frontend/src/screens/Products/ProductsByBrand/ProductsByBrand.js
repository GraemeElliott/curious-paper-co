...
function ProductsByBrand() {
  const urlBrand = useLocation().pathname.split('/')[3];
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const result = await axios.get(`/products/brands/${urlBrand}/`);
  ...
  return (
    <div>
      ...
      <h1 className="product-brand-title">{urlBrand.replace(/-/g, ' ')}</h1>
